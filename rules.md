...
At any time, one player is the Speaker. The Speaker is responsible for updating
the list of active rules and proposals as necessary.

## 104

The Speaker for the first game shall be mwpuppire.
...
All rule-changes legally proposed shall be voted on. They will be adopted if and
only if they receive the required number of votes and quorum is achieved.

## 107

All proposed rule-changes shall be posted in the `#voting` channel before they
...
The Speaker shall give each proposed rule change a number for reference. The
numbers shall begin with 301, and each rule-change legally proposed shall
...
_n_ points to any other state of affairs. The magnitude of _n_ and the means of
...
Members of this Discord server who are not bots are considered players. A player
may opt out at any time from being considered a player by making some
declaration to that effect, and likewise can opt in by the same method.
...
All players begin with zero points. Points may not be gained, lost, or moved
except as explicitly stated in the rules.

## 203

One turn consists of two parts in this order: (1) proposing one rule-change and
having it voted on, and (2) subtracting 291 from the ordinal number of the
proposal and multiplying the result by the fraction of favorable votes it
received, rounded to the nearest integer, and adding this to the player's score.
...
A quorum is considered to be half of all players. No vote on a rule-change is
considered valid or finalized until at least a quorum of players have
participated in it.

This means no turn is considered completed until a quorum of players have
participated in any required votes.
...
Votes are made in the `#voting` channel, by making some declaration to the
effect of affirming or rejecting a proposal. Votes may not be cast by any other
means.

## 211

The winner is the first player to achieve 200 (positive) points. When a player
wins in this way:

* that player becomes the Speaker,
* all players' scores are reset to 0,
* and a new game is begun. All rules and proposed rule changes retain the status
  they had at the end of the prior game.
...
application of a rule, then a player may invoke Judgment by making a declaration
to that effect. Disagreement, for the purposes of this rule, may be created by
the insistence of any player.

When Judgment has been invoked, the next player may not begin their turn
...
for the question, and so on, except that no player is to be Judge during their
own turn or during the turn of a team-mate.

Unless a Judge is overruled, one Judge settles all questions arising from the
game until the next turn is begun, including questions as to their own
...
that affect the completion of the turn in which Judgment was invoked.

## 215

When Judgment is invoked, one player is randomly selected to be the Judge. If
the Judge fails to deliver a judgment within this time, they are penalized 10
points and a new Judge is appointed.
...
issue, then the Judge shall consider game-custom and the spirit of the game
before applying other standards.

## 217

A player's turn is considered to begin when either:

* the player takes their first legal action as part of their turn,
* or the Speaker declares their turn is next.

## 218

If a turn has not been completed within 72 hours, the Speaker may choose at any
time to end the turn. If it is unclear how to proceed to the next turn, the
Speaker may call for Judgment before the next player begins.
...
first player unable to complete a turn is the winner, with no provision for
...
In addition to duties listed elsewhere in the rules, the Speaker has the
following duties:

* keeping an up-to-date list of players and their current scores, and making
  such list available to players.
* keeping an up-to-date list of the current rules, and making such list
